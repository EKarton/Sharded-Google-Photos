...
class GPhotosCleaner:
    ...
    def mark_unalbumed_photos_to_trash(self):
        ...
        trash_album = None
        for album in self.gphoto_client.list_albums():
            if album["title"] == "Trash":
                trash_album = album

        if trash_album is None:
            logger.debug(f"No trash album found. Creating new trash album")
            trash_album = self.gphoto_client.create_album("Trash")
        ...
        for shared_album in self.gphoto_client.list_shared_albums():
            shared_album_id = shared_album["id"]
            for media_items in self.gphoto_client.search_for_media_items(
                shared_album_id
            ):
        ...
        for media_item in self.gphoto_client.search_for_media_items():
        ...
        if len(media_item_ids_to_trash) > 0:
            self.gphoto_client.add_photos_to_album(
